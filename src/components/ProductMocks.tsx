...
export const BrowserChrome: React.FC<{ url?: string; children: React.ReactNode }> = ({
  ...
  <div className="ay-mock overflow-hidden rounded-[20px] border border-[#D9E2EA] bg-[#FFFFFF]">
    <div className="flex items-center gap-2 px-3.5 sm:px-4 py-2.5 border-b border-[#D9E2EA] bg-[#FFFFFF]">
      ...
      <div className="ml-2 flex-1 min-w-0 h-6 rounded-full bg-[#FFFFFF] border border-[#D9E2EA] px-3 flex items-center">
...
export const PhoneChrome: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <div className="ay-mock mx-auto max-w-[340px] rounded-[32px] border-[6px] border-[#10203A] bg-[#FFFFFF] overflow-hidden">
    ...
    <BrowserChrome url="app.afteryes.com/radar">
      <div className="flex items-center justify-between pb-4 border-b border-[#D9E2EA] mb-4">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-full bg-[#E8F4F4] flex items-center justify-center text-[#E25A48] font-serif font-bold text-sm">AY</div>
          <div>
            <h3 className="text-base font-serif text-[#10203A]">Client Retention Radar</h3>
      ...
      <div className="space-y-2.5 text-sm">
        {rows.map((row) => (
          <div
            key={row.name}
            className={`p-3.5 rounded-full flex items-center justify-between gap-3 ay-rise-row ${
              row.status === 'flag'
                ? 'bg-[#FFFFFF] border border-[#E25A48] ay-flag-in'
                : 'bg-[#FFFFFF] border border-[#D9E2EA]'
            }`}
          ...
          >
            ...
            {row.status === 'flag' ? (
              ...
              <span className="text-xs text-[#E25A48] font-medium flex items-center gap-1 shrink-0">
...
export const WhatsAppRecapMock: React.FC = () => (
  <PhoneChrome>
    <div className="space-y-3">
      ...
      <div className="max-w-[70%] ml-auto rounded-[16px] rounded-tr-sm bg-[#FFFFFF] px-3.5 py-2.5 text-[13px] text-[#5C6B80] ay-bubble ay-bubble-delay shadow-sm">
        …
      </div>
...
export const SessionNoteMock: React.FC = () => (
  <BrowserChrome url="app.afteryes.com/sessions/marcus">
    ...
    <div className="space-y-3 text-sm">
      <div className="flex justify-between border-b border-[#D9E2EA] pb-2.5">
        <span className="text-[#5C6B80]">Client</span>
        <span className="text-[#10203A] font-medium">Marcus V.</span>
      </div>
      <div className="flex justify-between border-b border-[#D9E2EA] pb-2.5">
        <span className="text-[#5C6B80]">Covered</span>
        <span className="text-[#10203A] text-right">Protein target + sleep window</span>
      </div>
      <div className="flex justify-between border-b border-[#D9E2EA] pb-2.5">
        <span className="text-[#5C6B80]">Commitment</span>
        <span className="text-[#10203A] text-right">Walk 7am · 4 days</span>
      </div>
      <div className="flex justify-between">
        <span className="text-[#5C6B80]">Next</span>
        <span className="text-[#E25A48] font-medium">Thu 4:00pm · WhatsApp</span>
      </div>
    </div>
  ...
  <BrowserChrome url="app.afteryes.com/practice">
    <div className="flex items-center justify-between pb-4 border-b border-[#D9E2EA] mb-4">
      <div className="flex items-center gap-3">
        <div className="w-9 h-9 rounded-full bg-[#E8F4F4] flex items-center justify-center text-[#E25A48] font-serif font-bold text-sm">AY</div>
        <div>
          <h3 className="text-base font-serif text-[#10203A]">Practice Calendar & Call Queue</h3>
    ...
    <div className="space-y-2.5 text-sm">
      <div className="p-3.5 bg-[#FFFFFF] rounded-full border border-[#D9E2EA] flex items-center justify-between gap-3 ay-slot">
        <div>
      ...
      <div className="p-3.5 bg-[#FFFFFF] rounded-full border border-[#E25A48] flex items-center justify-between gap-3">
        <div>
      ...
      <div className="p-3.5 bg-[#FFFFFF] rounded-full border border-[#D9E2EA] flex items-center justify-between gap-3">
        <div>
          <p className="font-medium text-[#10203A]">Thu 10:30 · New patient hygiene</p>
          <p className="text-xs text-[#5C6B80]">Confirmed 48h and 2h out</p>
        </div>
        <span className="text-xs text-[#E25A48] font-medium shrink-0">Confirmed</span>
      </div>
    </div>
  </BrowserChrome>
);
...
export const VoiceDeskMock: React.FC = () => (
  <BrowserChrome url="app.afteryes.com/voice">
    ...
    <div className="rounded-[16px] border border-[#D9E2EA] bg-[#FFFFFF] p-4">
      <p className="text-xs text-[#5C6B80] mb-2">Live call · 7:14pm</p>
      <p className="text-sm text-[#10203A] leading-relaxed">
...
export const ServiceStory: React.FC<StoryProps> = ({ eyebrow, title, body, points, mock, flip }) => (
  <div className={`grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center ${flip ? 'lg:[&>*:first-child]:order-2' : ''}`}>
    <div>
      ...
      <h3 className="text-2xl sm:text-[2rem] font-serif text-[#10203A] mb-4 leading-[1.2]">{title}</h3>
      ...
      <ul className="space-y-3">
        {points.map((p) => (
          <li key={p} className="flex items-start gap-2.5 text-sm text-[#10203A]">
            <Check className="w-4 h-4 text-[#E25A48] mt-0.5 shrink-0" />
            <span>{p}</span>
          </li>
        ))}
      </ul>
