    ...
    <footer className="w-full bg-[#10203A] pt-16 pb-14 mt-0 text-white">
      <div className="max-w-[1120px] mx-auto px-4 sm:px-6">
        <div className="bg-white rounded-[20px] p-6 sm:p-8 mb-16 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 text-[#10203A]">
          <div>
            <div className="text-xs uppercase tracking-[0.12em] font-sans font-medium text-[#5C6B80] mb-1.5">
              Current view: {isClinic ? 'Clinic' : '1:1 Coach'}
            </div>
            <h3 className="text-xl font-sans font-bold text-[#10203A]">
              {isClinic ? 'Need retention tools for 1:1 coaching instead?' : 'Looking for scheduling and after-hours call flow for a clinic?'}
            </h3>
            <p className="text-sm text-[#5C6B80] mt-1">One site, two paths. Switch any time.</p>
          </div>
          <button onClick={() => handleSwitch(isClinic ? 'coach' : 'clinic')} id="footer-switch-audience-btn" className="shrink-0 inline-flex items-center gap-2 px-[22px] py-[14px] rounded-full bg-white hover:bg-[#F3F6F8] border-2 border-[#10203A] text-[#10203A] text-sm font-medium transition-colors cursor-pointer">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8 mb-12">
          ...
          <div className="space-y-3">
            <div className="text-xs uppercase tracking-[0.12em] font-medium text-white/50">Paths</div>
            <ul className="space-y-2 text-sm text-white">
              <li><button onClick={() => handleSwitch('coach')} className="hover:text-[#E25A48] transition-colors cursor-pointer text-left">For 1:1 coaches ($29/mo)</button></li>
              <li><button onClick={() => handleSwitch('clinic')} className="hover:text-[#E25A48] transition-colors cursor-pointer text-left">For clinics ($99/mo)</button></li>
            </ul>
          </div>
          <div className="space-y-3">
            <div className="text-xs uppercase tracking-[0.12em] font-medium text-white/50">Legal & Contact</div>
            <ul className="space-y-2 text-sm text-white">
              <li><button onClick={() => navigate('/privacy')} className="hover:text-[#E25A48] transition-colors cursor-pointer text-left">Privacy</button></li>
              <li><button onClick={() => navigate('/terms')} className="hover:text-[#E25A48] transition-colors cursor-pointer text-left">Terms</button></li>
              <li>
